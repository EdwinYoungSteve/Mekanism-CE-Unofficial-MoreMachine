...
public class GuiTierRotaryCondensentrator extends GuiMekanismTile<TileEntityTierRotaryCondensentrator> implements IJeiNoShowRecipe {
    ...
    public GuiTierRotaryCondensentrator(InventoryPlayer inventory, TileEntityTierRotaryCondensentrator tile) {
        ...
        addGuiElement(new GuiInputSlot(this, resource, 4, 24, tileEntity).with(GuiSlot.SlotOverlay.PLUS));
        addGuiElement(new GuiOutputSlot(this, resource, 4, 55, tileEntity).with(GuiSlot.SlotOverlay.MINUS));
        ...
        }, GuiProgress.ProgressBar.LARGE_LEFT, this, resource, 62, 38));
    ...
    public void initGui() {
        super.initGui();
        buttonList.clear();
        buttonList.add(mode = new GuiDisableableButton(0, guiLeft + 4, guiTop + 4, 18, 18, () -> tileEntity.mode ? 0 : 1).with(GuiDisableableButton.ImageOverlay.TOGGLE));
    }

    @Override
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        fontRenderer.drawString(tileEntity.getName(), (xSize / 2) - (fontRenderer.getStringWidth(tileEntity.getName()) / 2), 4, 0x404040);
        fontRenderer.drawString(tileEntity.mode ? LangUtils.localize("gui.condensentrating")
        ...
        if (mode.isMouseOver()) {
            ...
            boolean gas = tileEntity.gasTank.getStored() == tileEntity.gasTank.getMaxGas() && !tileEntity.mode;
            boolean fluid = tileEntity.fluidTank.getFluidAmount() == tileEntity.fluidTank.getCapacity() && tileEntity.mode;
            if (energy) {
